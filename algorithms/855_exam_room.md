 ...
 * @LastEditors: Hiseh
 * @LastEditTime: 2019-12-12 18:05:17
 * @Description: 
 -->
# 考场就座 
[返回首页](../README.md)
...
因为一开始就告诉了教室大小，所以可以用数组来存储学生座位，每次就座时通过二分法查找座位，这种方式效率高*ϴ(ln<sup>N</sup>)*，但空间占用大*ϴ(N)*。考虑到学生就座次数比座位数少5个数量级（座位数：*1 <= N <= 10<sup>9</sup>*，就座次数：*< 10<sup>4</sup>*），完全可以用哈希表/链表实现，时间复杂度高*ϴ(N)*，但更节约内存。
## Python
采用第二种思路
```python
class ExamRoom:
