<!--
 * @Author: Hiseh
 * @Date: 2019-03-18 11:04:14
 * @LastEditors: Hiseh
 * @LastEditTime: 2019-10-22 21:23:15
 * @Description: 
 -->
# 汇总区间
[返回首页](../README.md)<br>
...
## C
C里虽然对数组引用也是通过指针引用的，但C中内存不如Python灵活，如果手动申请（通过`malloc`/`calloc`/`realloc`），那么result中插入的tmp对应内存地址也是指定的，没办法动态绑定。如果自动申请（通过数组方式），则需要事先知道tmp长度，当然可以指定tmp长度与nums长度一样，这么做会浪费太多内存空间，最坏情况会浪费掉 *4(n<sup>2</sup>-n)* 字节。
